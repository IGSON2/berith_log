...
func TestBlockReceiptStorage(t *testing.T) {
	...
	tx1 := types.NewTransaction(1, common.HexToAddress("0x1"), big.NewInt(1), 1, big.NewInt(1), nil, 1, 1)
	tx2 := types.NewTransaction(2, common.HexToAddress("0x2"), big.NewInt(2), 2, big.NewInt(2), nil, 1, 1)
	...
	if rs := ReadReceipts(db, hash, 0, params.TestnetChainConfig); len(rs) != 0 {
	...
	DeleteBody(db, hash, 0)
	if rs := ReadReceipts(db, hash, 0, params.TestnetChainConfig); rs != nil {

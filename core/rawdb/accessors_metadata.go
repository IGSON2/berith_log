...
func ReadDatabaseVersion(db berithdb.KeyValueReader) *uint64 {
	var version uint64
...
func WriteDatabaseVersion(db berithdb.KeyValueWriter, version uint64) {
...
func ReadChainConfig(db berithdb.KeyValueReader, hash common.Hash) *params.ChainConfig {
...
func WriteChainConfig(db berithdb.KeyValueWriter, hash common.Hash, cfg *params.ChainConfig) {
	if cfg == nil {
		return
	}
...
func PushUncleanShutdownMarker(db berithdb.KeyValueStore) ([]uint64, uint64, error) {
...
func PopUncleanShutdownMarker(db berithdb.KeyValueStore) {
	var uncleanShutdowns crashList
	// Read old data
	if data, err := db.Get(uncleanShutdownKey); err != nil {
		log.Warn("Error reading unclean shutdown markers", "error", err)

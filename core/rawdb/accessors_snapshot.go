...
func ReadSnapshotDisabled(db berithdb.KeyValueReader) bool {
	disabled, _ := db.Has(snapshotDisabledKey)
	return disabled
}

// WriteSnapshotDisabled stores the snapshot pause flag.
func WriteSnapshotDisabled(db berithdb.KeyValueWriter) {
...
func DeleteSnapshotDisabled(db berithdb.KeyValueWriter) {
...
func ReadSnapshotRoot(db berithdb.KeyValueReader) common.Hash {
...
// the persisted snapshot.
func WriteSnapshotRoot(db berithdb.KeyValueWriter, root common.Hash) {
...
// invalid.
func DeleteSnapshotRoot(db berithdb.KeyValueWriter) {
...
func ReadAccountSnapshot(db berithdb.KeyValueReader, hash common.Hash) []byte {
	data, _ := db.Get(accountSnapshotKey(hash))
	return data
}

// WriteAccountSnapshot stores the snapshot entry of an account trie leaf.
func WriteAccountSnapshot(db berithdb.KeyValueWriter, hash common.Hash, entry []byte) {
...
func DeleteAccountSnapshot(db berithdb.KeyValueWriter, hash common.Hash) {
...
func ReadStorageSnapshot(db berithdb.KeyValueReader, accountHash, storageHash common.Hash) []byte {
	data, _ := db.Get(storageSnapshotKey(accountHash, storageHash))
	return data
}

// WriteStorageSnapshot stores the snapshot entry of an storage trie leaf.
func WriteStorageSnapshot(db berithdb.KeyValueWriter, accountHash, storageHash common.Hash, entry []byte) {
...
func DeleteStorageSnapshot(db berithdb.KeyValueWriter, accountHash, storageHash common.Hash) {
...
func IterateStorageSnapshots(db berithdb.Iteratee, accountHash common.Hash) berithdb.Iterator {
	return db.NewIterator(storageSnapshotsKey(accountHash), nil)
}

// ReadSnapshotJournal retrieves the serialized in-memory diff layers saved at
// the last shutdown. The blob is expected to be max a few 10s of megabytes.
func ReadSnapshotJournal(db berithdb.KeyValueReader) []byte {
...
func WriteSnapshotJournal(db berithdb.KeyValueWriter, journal []byte) {
...
// the last shutdown
func DeleteSnapshotJournal(db berithdb.KeyValueWriter) {
...
// the last shutdown.
func ReadSnapshotGenerator(db berithdb.KeyValueReader) []byte {
...
// shutdown.
func WriteSnapshotGenerator(db berithdb.KeyValueWriter, generator []byte) {
...
// the last shutdown
func DeleteSnapshotGenerator(db berithdb.KeyValueWriter) {
...
// snapshot layer.
func ReadSnapshotRecoveryNumber(db berithdb.KeyValueReader) *uint64 {
...
// snapshot layer.
func WriteSnapshotRecoveryNumber(db berithdb.KeyValueWriter, number uint64) {
	var buf [8]byte
...
// snapshot layer.
func DeleteSnapshotRecoveryNumber(db berithdb.KeyValueWriter) {
...
func ReadSnapshotSyncStatus(db berithdb.KeyValueReader) []byte {
	data, _ := db.Get(snapshotSyncStatusKey)
	return data
}

// WriteSnapshotSyncStatus stores the serialized sync status to save at shutdown.
func WriteSnapshotSyncStatus(db berithdb.KeyValueWriter, status []byte) {
...
// shutdown
func DeleteSnapshotSyncStatus(db berithdb.KeyValueWriter) {

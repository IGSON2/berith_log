...
func generateSnapshot(diskdb berithdb.KeyValueStore, triedb *trie.Database, cache int, root common.Hash) *diskLayer {
...
func journalProgress(db berithdb.KeyValueWriter, marker []byte, stats *generatorStats) {
...
func (dl *diskLayer) generateRange(root common.Hash, prefix []byte, kind string, origin []byte, max int, stats *generatorStats, onState onStateCallback, valueConvertFn func([]byte) ([]byte, error)) (bool, []byte, error) {
	...
	var snapNodeCache berithdb.KeyValueStore
	if len(result.keys) > 0 {
...
func (dl *diskLayer) generate(stats *generatorStats) {
	...
	checkAndFlush := func(currentLocation []byte) error {
		...
		if batch.ValueSize() > berithdb.IdealBatchSize || abort != nil {

...
func TestTransactionPriceNonceSort(t *testing.T) {
	...
	for start, key := range keys {
		addr := crypto.PubkeyToAddress(key.PublicKey)
		for i := 0; i < 25; i++ {
			tx, _ := SignTx(NewTransaction(uint64(start+i), common.Address{}, big.NewInt(100), 100, big.NewInt(int64(start+i)), nil, 1, 1), signer, key)
...
func TestTransactionTimeSort(t *testing.T) {
	...
	for start, key := range keys {
		...
		tx, _ := SignTx(NewTransaction(0, common.Address{}, big.NewInt(100), 100, big.NewInt(1), nil, 1, 1), signer, key)

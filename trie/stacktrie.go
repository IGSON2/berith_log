...
func stackTrieFromPool(db berithdb.KeyValueWriter) *StackTrie {
...
type StackTrie struct {
	...
	db        berithdb.KeyValueWriter // Pointer to the commit db, can be nil
}

// NewStackTrie allocates and initializes an empty trie.
func NewStackTrie(db berithdb.KeyValueWriter) *StackTrie {
	return &StackTrie{
		nodeType: emptyNode,
		db:       db,
	}
...
func NewFromBinary(data []byte, db berithdb.KeyValueWriter) (*StackTrie, error) {
	var st StackTrie
...
func (st *StackTrie) setDb(db berithdb.KeyValueWriter) {
	st.db = db
	for _, child := range st.children {
		if child != nil {
			child.setDb(db)
...
func newLeaf(ko int, key, val []byte, db berithdb.KeyValueWriter) *StackTrie {
...
func newExt(ko int, key []byte, child *StackTrie, db berithdb.KeyValueWriter) *StackTrie {
	st := stackTrieFromPool(db)
	st.nodeType = extNode
	st.keyOffset = ko
	st.key = append(st.key, key[ko:]...)

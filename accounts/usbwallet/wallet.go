...
type wallet struct {
	...
	deriveChain     berith.ChainStateReader   // Blockchain state reader to discover used account with
...
// chain state reader.
func (w *wallet) SelfDerive(bases []accounts.DerivationPath, chain berith.ChainStateReader) {
	w.stateLock.Lock()
	defer w.stateLock.Unlock()

...
func createGQLService(t *testing.T, stack *node.Node) {
	...
	ethConf := &berith.Config{
		Genesis: &core.Genesis{
			Config:     params.AllbsrrProtocolChanges,
			GasLimit:   11500000,
			Difficulty: big.NewInt(1048576),
		},
		bsrr: bsrr.Config{
			PowMode: bsrr.ModeFake,
		},
		NetworkId:               1337,
		TrieCleanCache:          5,
	...
	chain, _ := core.GenerateChain(params.AllbsrrProtocolChanges, ethBackend.BlockChain().Genesis(),
		bsrr.NewFaker(), ethBackend.ChainDb(), 10, func(i int, gen *core.BlockGen) {})
...
func createGQLServiceWithTransactions(t *testing.T, stack *node.Node) {
	...
	// Create some blocks and import them
	chain, _ := core.GenerateChain(params.AllbsrrProtocolChanges, ethBackend.BlockChain().Genesis(),
		bsrr.NewFaker(), ethBackend.ChainDb(), 1, func(i int, b *core.BlockGen) {
